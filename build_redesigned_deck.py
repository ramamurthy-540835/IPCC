...
tb(s, 0.75, 1.35, 8.0, 0.3, "IPCC CLIMATE AI ASSISTANT  ·  GOOGLE CLOUD", 12, RGBColor(0x6F, 0xD0, 0xC2), True, spc=200)
...
# ================================================================ SLIDE 4 — WHAT (hub & spoke)
s = slide_new()
header(s, "What it does", "One assistant, five ways to work with climate reports",
       "Every workflow runs through the same conversation — no tooling change, no training curve.", 4)
...
footer(s, "Interface: Streamlit   ·   Backend: Python services   ·   Intelligence: Vertex AI")
...
# ================================================================ SLIDE 5 — ARCHITECTURE (layered landscape)
s = slide_new()
header(s, "How it is built", "A three-layer platform: knowledge, intelligence, experience",
       "Managed GCP services separate storage, retrieval, reasoning and presentation — nothing to patch, everything scales.", 5)
...
pill(8.6, 3.9, 2.7, "Gemini 2.5 Flash", "grounded generation", GREEN)
...
# ================================================================ SLIDE 6 — RAG JOURNEY (metro line)
s = slide_new()
header(s, "How answers are made", "The journey from question to defensible answer",
       "Five stations — retrieval comes before generation, so the model explains evidence instead of inventing it.", 6)
...
footer(s, "Same pattern for all five workflows — only the prompt changes.")
...
# ================================================================ SLIDE 7 — PROOF (KPI dashboard)
s = slide_new()
header(s, "Where it stands", "Live and verified in project ai-ippc",
       "Every figure below was read from the deployed environment — not from a plan.", 7, accent=GREEN)
...
tiles = [
    ...
    ("MODELS", "2", "Vertex AI models", ["Gemini 2.5 Flash", "text-embedding-005", "Region us-central1"], GREEN),
]
tx = 0.6
...
# ================================================================ SLIDE 8 — RESILIENCE (incident timeline)
s = slide_new()
header(s, "What we learned", "One incident, resolved — and the platform is stronger for it",
       "A launch-day outage was diagnosed in logs, fixed at the root cause, and verified from the outside.", 8, accent=AMBER)
...
# ================================================================ SLIDE 9 — ROADMAP (staircase)
s = slide_new()
header(s, "Where we go next", "From working deployment to production climate intelligence",
       "Each step compounds the last — validate what exists, then harden, secure, operate and scale it.", 9, accent=GREEN, hsize=25)
...
footer(s, "IPCC AI — evidence-led climate knowledge, delivered through managed GCP services.", accent=GREEN)
...
# ---------------------------------------------------------------- save
prs.core_properties.title = "IPCC Climate AI Assistant — Executive Overview"
prs.core_properties.author = "IPCC AI / GCP Studio"
out = "/home/appadmin/GCP-Studio/IPCC/IPCC_AI_GCP_Overview_v2.0_Redesigned.pptx"
prs.save(out)
print("saved", out)
